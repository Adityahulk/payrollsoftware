...
export default function AttendancePage({ isAdmin }) {
  ...
  const fetchData = async () => {
    ...
    try {
      if (isAdmin) {
        ...
        const employees = usersData.filter(u => (u.role || '').toLowerCase() !== 'admin');
        if (employees.length > 0) {
          setSelectedEmpId(employees[0].empId || employees[0].id);
        }
      } else {
  ...
  const employees = useMemo(() => {
    return allUsers.filter(u => (u.role || '').toLowerCase() !== 'admin');
  }, [allUsers]);
  ...
  const stats = useMemo(() => {
    ...
    const lateCount = records.filter(r => (r.lateMinutes || r.lateminutes || 0) > 0).length;
    const earlyExitCount = records.filter(r => (r.earlyExitMinutes || r.earlyexitminutes || 0) > 0).length;
  ...
  }, [records, dateOfJoining, workingDays]);
  ...
  const selectedStats = useMemo(() => {
    const presentCount = selectedEmpAttendance.length;
    const lateCount = selectedEmpAttendance.filter(r => (r.lateMinutes || r.lateminutes || 0) > 0).length;
    const earlyExitCount = selectedEmpAttendance.filter(r => (r.earlyExitMinutes || r.earlyexitminutes || 0) > 0).length;
    ...
    const absentCount = Math.max(0, totalDays - presentCount);
    return { presentCount, absentCount, lateCount, earlyExitCount, totalDays };
  }, [selectedEmpAttendance, selectedEmpDoj, selectedEmpWorkingDays]);
      ...
      <AppLayout role={user?.role === 'Admin' ? 'admin' : 'employee'}>
        <div className="page-content fade-in">
          ...
          <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start' }}>
            {/* Left side: Today's list */}
            <div className="card" style={{ width: '320px', flexShrink: 0, padding: 18, background: '#fff' }}>
              <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 14 }}>Today's Roll Call</h3>
              
              {/* List Search Bar */}
              <div style={{ display: 'flex', alignItems: 'center', background: 'var(--gray-100)', border: '1px solid var(--gray-200)', borderRadius: 'var(--radius-md)', padding: '6px 12px', marginBottom: 16 }}>
              ...
              <div style={{ marginBottom: 20 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--gray-600)', textTransform: 'uppercase', letterSpacing: '.04em' }}>Present Today</span>
                  </div>
                  <span className="badge badge-success" style={{ fontSize: 10 }}>{presentTodayList.length}</span>
                </div>
                <div style={{ maxHeight: '250px', overflowY: 'auto', paddingRight: 4 }}>
                  {presentTodayList.length === 0 ? (
                    <div style={{ padding: 12, textAlign: 'center', color: 'var(--gray-400)', fontSize: 12, border: '1px dashed var(--gray-200)', borderRadius: 6 }}>No employees present</div>
                  ) : (
                    presentTodayList.map(emp => {
                      ...
                      return (
                        <div 
                          ...
                          onMouseEnter={(e) => !isActive && (e.currentTarget.style.backgroundColor = 'var(--gray-50)')}
                          onMouseLeave={(e) => !isActive && (e.currentTarget.style.backgroundColor = 'transparent')}
                        >
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <div className="avatar" style={{ width: 28, height: 28, fontSize: 10 }}>{getInitials(emp.name || emp.email)}</div>
                            <div style={{ minWidth: 0 }}>
                              <div style={{ fontSize: 12, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 120 }}>{emp.name || emp.email}</div>
                              <div style={{ fontSize: 10, color: 'var(--gray-400)' }}>{emp.role}</div>
                            </div>
                          </div>
                          {clockInTime && (
                            <span style={{ fontSize: 10, color: 'var(--success)', fontWeight: 600, fontFamily: 'JetBrains Mono' }}>{clockInTime}</span>
                          )}
                        </div>
                      );
                    })
              ...
              {/* Not Present Today list */}
              <div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--gray-400)' }} />
                    <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--gray-600)', textTransform: 'uppercase', letterSpacing: '.04em' }}>Not Present</span>
                  </div>
                  <span className="badge badge-gray" style={{ fontSize: 10 }}>{absentTodayList.length}</span>
                </div>
                <div style={{ maxHeight: '250px', overflowY: 'auto', paddingRight: 4 }}>
                  {absentTodayList.length === 0 ? (
                    <div style={{ padding: 12, textAlign: 'center', color: 'var(--gray-400)', fontSize: 12, border: '1px dashed var(--gray-200)', borderRadius: 6 }}>Everyone is clocked in</div>
                  ) : (
                      ...
                      return (
                        ...
                        >
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <div className="avatar" style={{ width: 28, height: 28, fontSize: 10 }}>{getInitials(emp.name || emp.email)}</div>
                            <div style={{ minWidth: 0 }}>
                              <div style={{ fontSize: 12, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 140 }}>{emp.name || emp.email}</div>
                              <div style={{ fontSize: 10, color: 'var(--gray-400)' }}>{emp.role}</div>
                            </div>
                          </div>
            ...
            {/* Right side: Selected Employee calendar & details */}
            <div style={{ flex: 1 }}>
              {/* Dropdown Employee Search selector */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, background: '#fff', padding: 14, borderRadius: 'var(--radius-lg)', border: '1px solid var(--gray-200)', boxShadow: 'var(--shadow-sm)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--gray-600)' }}>Select Employee:</span>
                  ...
                  <div style={{ position: 'relative', width: 280 }}>
                    <div style={{ display: 'flex', alignItems: 'center', background: 'var(--gray-100)', border: '1.5px solid var(--gray-200)', borderRadius: 'var(--radius-md)', padding: '0 10px' }}>
                      ...
                      <input 
                        ...
                        style={{ border: 'none', padding: '8px 0', outline: 'none', fontSize: 13, width: '100%', background: 'transparent' }}
                      />
                      {selectSearch && (
                        <button 
                ...
                {activeEmployee && (
                  <div style={{ fontSize: 13, color: 'var(--gray-500)', display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <span className="badge badge-primary">{activeEmployee.role}</span>
                  </div>
                )}
              </div>
                ...
                <>
                  ...
                  <div className="grid grid-4" style={{ marginBottom: 24 }}>
                    {[
                      ...
                      <div key={c.label} className="stat-card" style={{ padding: 14 }}>
                        ...
                        <div style={{ fontSize: 11, color: 'var(--gray-400)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.06em' }}>{c.label}</div>
                        ...
                        <div className="progress-bar" style={{ marginTop: 6 }}>
                          <div className="progress-fill" style={{ width: `${selectedStats.totalDays ? Math.min((c.value / selectedStats.totalDays) * 100, 100) : 0}%`, background: c.color, transition: 'width .6s ease' }} />
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 20 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                      ...
                      {/* Recent Records list for selected employee */}
                      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                ...
                <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
                  {[{ mon: 'MAY', day: '19', name: 'Buddha Purnima', type: 'National Holiday' }, { mon: 'JUN', day: '15', name: 'Eid al-Adha', type: 'Regional Holiday' }].map(h => (
                    <div key={h.name} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ width: 44, height: 44, background: 'var(--primary-50)', borderRadius: 8, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--primary-500)', fontFamily: 'JetBrains Mono' }}>{h.mon}</span>
                        <span style={{ fontSize: 17, fontWeight: 800, color: 'var(--primary-700)', lineHeight: 1 }}>{h.day}</span>
                      </div>
                      <div>
